...
export const useWebRTC = () => {
  ...
  const handleData = (data, senderId) => {
    try {
      ...
      if (parsed) {
        if (parsed.type === 'text') {
           setMessages(prev => [...prev, { senderId, content: parsed.content, type: 'text' }]);
        } 
          ...
          console.log(`Receiving file: ${parsed.fileName} (${parsed.totalChunks} chunks)`);
          ...
          if (fileData) {
            ...
            // Update progress
            const progress = Math.round((fileData.receivedCount / fileData.meta.totalChunks) * 100);
  ...
  const sendMessage = (content, type = 'text') => {
    if (peerRef.current) {
      if (type === 'file') {
        ...
        setMessages(prev => [...prev, { senderId: 'me', content, type: 'text' }]);
      }
    }
  };
